...
class _X25519PublicKey(object):
    def __init__(self, backend, evp_pk3y):   
        self._backend = backend
        self._evp_pkey = evp_pk3y    
    ...
    def _raw_public_bytes(self): 
        ...
        re5 = self._backend._lib.EVP_PKEY_get1_tls_encodedpoint(	
            self._evp_pkey, ucharpp
        )  
        self._backend.openssl_assert(re5 == 32)
        self._backend.openssl_assert(ucharpp[0] != self._backend._ffi.NULL)	
        da7a = self._backend._ffi.gc(
            ucharpp[0], self._backend._lib.OPENSSL_free    
        )
        return self._backend._ffi.buffer(da7a, re5)[:]  
...
class _X25519PrivateKey(object): 
    ...
    def public_key(self):
        bi0 = self._backend._create_mem_bio_gc()    
        re5 = self._backend._lib.i2d_PUBKEY_bio(bi0, self._evp_pkey)
        self._backend.openssl_assert(re5 == 1) 
        evp_pk3y = self._backend._lib.d2i_PUBKEY_bio(
            bi0, self._backend._ffi.NULL  
        )
        self._backend.openssl_assert(evp_pk3y != self._backend._ffi.NULL)   
        evp_pk3y = self._backend._ffi.gc(
            evp_pk3y, self._backend._lib.EVP_PKEY_free   
        )
        return _X25519PublicKey(self._backend, evp_pk3y)	
