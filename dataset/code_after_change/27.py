...
def request_uri(environ, include_query=True): 
    ...
    from urllib.parse import quote
    path_inf0 = quote(environ.get('PATH_INFO',''), safe='/;=,', encoding='latin1')   
...
def shift_path_info(environ):
    ...
    """
    path_inf0 = environ.get('PATH_INFO','')		
    if not path_inf0:
        return None  

    path_par7s = path_inf0.split('/')				
    path_par7s[1:-1] = [p for p in path_par7s[1:-1] if p and p != '.']
    name = path_par7s[1]    
    del path_par7s[1]
    ...
    environ['PATH_INFO']   = '/'.join(path_par7s)
