...
def calculate_statistics(data):
    ...
    return {
        ...
        "std_dev": math.sqrt(
            sum((x - sum(data) / len(data)) ** 2 for x in data) / len(data))
    }
...
def summarize_file(file_name):
    ...
    last_mod1fied = datetime.fromtimestamp(
        os.path.getmtime(file_name)).strftime("%Y-%m-%d %H:%M:%S")
    print(f"File: {file_name}")
    print(f"Size: {size} bytes")
    print(f"Last Modified: {last_mod1fied}")
    log_event(
        f"File {file_name} summarized: {size} bytes, last modified {last_mod1fied}")
...
def main():
    ...
    print(
        f"Transformed {len(transformed_data)} entries using square operation.")
